...
public class SkillState : MonoBehaviour {
    ...
    {
        private SkillSummon enemy;
        private Animator anim;

        private Vector3 player_pos;
        private Vector3 enemy_pos;

        public Idle(SkillSummon _enemy) : base("Idle")
        {
            enemy = _enemy;
            anim = enemy.GetAnim();
        }
    ...
    public class Movement : State
    {
        ...
        public Movement(SkillSummon _enemy) : base("Movement")
        {
            enemy = _enemy;
            anim = enemy.GetAnim();
        }
    ...
    public class Attack : State
    {
        ...
        public Attack(SkillSummon _enemy) : base("Attack")
        {
            enemy = _enemy;
            anim = enemy.GetAnim();
        }
        ...
        public override void Update()
        {
            if (particleDelay <= 0f)
            {
                ...
                if (Distance < enemy.MinDist + 0.5f)
                {
                    ...
                    if (Angle < 60f && Angle > -60f)
                    {
                        if(enemy.GetTarget().GetComponent<EnemyScript>() != null)
